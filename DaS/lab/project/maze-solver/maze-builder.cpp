...
// use the above cell struct to redefine a simpler maze
...
void generate_maze(struct maze_context *maze)
{
    ...
    { // 1x1 or less maze will make the code to go in an infinite loop so don't process those
        return;
    }
    ...
    while (1)
    {
        ...
        { // if all cells visited end the loop
            break;
        }
    }
